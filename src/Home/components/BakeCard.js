...
export default function BakeCard() {
  ...
  const fetchWalletBalance = async () => {
    ...
    try {
      const [bnbAmount, beansAmount, rewardsAmount, seedsAmount, referralAmount] = await Promise.all([
      ...
      const [replantRewards] = await Promise.all([
        contract.methods
            .calculateEggBuySimple(rewardsAmount)
            .call()
            .catch((err) => {
              console.error("calcreward", err);
              return 0;
            })
      ]);

      repRewards = replantRewards;
      ...
      <CardWrapper>
        ...
        <CardContent>
          ...
          >
            ...
            <Typography>{walletBalance.seeds} RUMORS</Typography>
          </Grid>
          <hr/>
          <Grid
              container
          ...
          >
            ...
            <Typography>{walletBalance.referral} RUMORS</Typography>
          </Grid>
          <hr />
